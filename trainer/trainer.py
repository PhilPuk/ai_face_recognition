...
def getImagesAndLabels(path):
    ...
    chooseSizeForTraining = int(len(id_pathes) * 1)
    ids = []
    face_Samples = []
    id = 1
...
def trainer(epochs, trainer_name):
    ...
    for i in range(0, epochs, 1):
        ...
        print(f"Epoch: {i}")
        recognizer.train(faces,np.array(ids))
        recognizer.write(f'{trainer_name}.yml')
        end_time = time.time()
...
faces ,ids,loading_time = getImagesAndLabelsPreProcessed(path, 1)
epochs = 0
training_time = 0
...
while True:
    ...
    if user_input == "1":
        training_time = trainer(epochs, "micro_model")
    elif user_input == "2":
